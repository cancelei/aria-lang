...
/// Usage: json_get(json: string, key: string) -> string
...
/// Convert an Aria Value to a serde_json::Value
...
mod tests {
    ...
    fn test_json_parse_invalid() {
        let json = r#"{"name":"aria""#; // Invalid JSON
        let result = json_parse(vec![Value::String(json.to_string())]);
    ...
    fn test_json_stringify() {
        ...
        let result = json_stringify(vec![Value::Number(42.0)]);
        assert_eq!(result, Ok(Value::String("42.0".to_string())));
    }
